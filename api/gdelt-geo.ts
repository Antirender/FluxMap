...
const UPSTREAM_HTTP  = 'http://api.gdeltproject.org/api/v2/geo/geo';
const UPSTREAM_HTTPS = 'https://api.gdeltproject.org/api/v2/geo/geo';
...
async function fetchWithRetry(qs: string, timeoutMs: number) {
  ...
  for (const url of urls) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        console.log(`[gdelt-geo] attempt ${attempt + 1} → ${url.slice(0, 60)}…`);
        return await httpGet(url, timeoutMs);
      } catch (e) {
        lastErr = e instanceof Error ? e : new Error(String(e));
        console.warn(`[gdelt-geo] attempt ${attempt + 1} failed:`, lastErr.message);
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('[gdelt-geo] all attempts failed:', msg);
